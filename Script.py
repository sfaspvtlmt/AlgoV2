...
def Search(api , Variables):
    atmStrike = currentStrike.currentStrike(api, Variables["Index"])
    print(atmStrike)
    # Search Scripts and Store them
   
  
    hedgeCE = api.searchscrip("NFO",Variables["Index"]  +" " +
                              str(atmStrike + Variables["HedgeStrike"]) + " CE")
    ...
    hedgePE = api.searchscrip("NFO", Variables["Index"] +" " +
                              str(atmStrike - Variables["HedgeStrike"]) + " PE")
    Store.strike['hedgePE'] = hedgePE['values'][0]['tsym']
    Store.token['hedgePE'] = hedgePE['values'][0]['token']

    CE = api.searchscrip("NFO", Variables["Index"] +" " + str(atmStrike + Variables["Strike"]) + " CE")
    Store.strike['CE'] = CE['values'][0]['tsym']
    Store.token['CE'] = CE['values'][0]['token']

    PE = api.searchscrip("NFO", Variables["Index"] +" " + str(atmStrike - Variables["Strike"]) + " PE")
    ...
    Store.stopLoss['CE'] = math.ceil(float(res['lp']) *(1+ Variables["StopLoss"]/100) )

    res = api.get_quotes("NFO", Store.token['PE'])
    Store.Price['PE'] = float(res['lp'])
    Store.stopLoss['PE'] = math.ceil(float(res['lp']) *(1+ Variables["StopLoss"]/100) )
    system('cls')
